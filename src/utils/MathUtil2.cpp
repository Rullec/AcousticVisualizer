...
bool cMathUtil::IsSame(const tVector4 &v1, const tVector4 &v2,
                       const FLOAT eps)
{
...
void cMathUtil::ThresholdOp(tVectorX &v, FLOAT threshold)
{
                                                         ...
                                                         const tVector4 &v1_)
{
    ...
    FLOAT theta = std::asin(rot_axis.norm()); //[-pi/2, pi/2]
...
FLOAT cMathUtil::Truncate(FLOAT num, int digits)
{
...
//     tMatrixX D = tMatrixX::Zero(4, num_friction_dirs);
//     FLOAT gap = 2 * M_PI / num_friction_dirs;
...
//     tVector Y_normal = tVector4(0, 1, 0, 0);
//     tVector axis = Y_normal.cross3(normal).normalized();
//     FLOAT theta = std::acos(Y_normal.dot(normal)); // [0, pi]
...
FLOAT cMathUtil::CalcConditionNumber(const tMatrixX &mat)
{
...
tVector4 cMathUtil::RayCastTri(const tVector4 &ori, const tVector4 &dir,
                                ...
                                const tVector4 &p3, FLOAT eps /* = 1e-5*/)
{
    tMatrix3 mat;
    ...
    FLOAT beta = res[0], gamma = res[1], t = res[2], alpha = 1 - beta - gamma;
...
tVector4 cMathUtil::RayCastPlane(const tVector4 &ray_ori,
                                  ...
                                  FLOAT eps /*= 1e-10*/)
{
    FLOAT t = -(plane_equation.segment(0, 3).dot(ray_ori.segment(0, 3)) +
                plane_equation[3]) /
 ...
 */
tMatrixX
cMathUtil::CartesianProduct(const std::vector<std::vector<FLOAT>> &lists)
{
    std::vector<std::vector<FLOAT>> result = CartesianProductVec(lists);
...
cMathUtil::CartesianProductVec(const std::vector<std::vector<FLOAT>> &lists)
{
 ...
 */
FLOAT cMathUtil::CalcDistanceFromPointToLine(const tVector3 &point,
                                             ...
                                             const tVector3 &line_end)
{
    ...
    FLOAT length = origin_2_point.dot(origin_2_end);
 ...
 *      d = plane[3]
 */
FLOAT cMathUtil::EvaluatePlane(const tVector4 &plane, const tVector4 &point)
{
    FLOAT sum = plane[3];
    for (int i = 0; i < 3; i++)
        sum += plane[i] * point[i];
 ...
 */
FLOAT cMathUtil::CalcPlanePointDist(const tVector4 &plane,
...
tVector4 cMathUtil::SampleFromPlane(const tVector4 &plane_equation)
{
    ...
    point[3] = 1;
    FLOAT residual = -plane_equation[3];
    for (int i = 0; i < 3; i++)
    {
        if (i != id)
        {
...
FLOAT cMathUtil::CalcTriangleArea(const tVector4 &p0, const tVector4 &p1,
                                  const tVector4 &p2)
{
...
FLOAT cMathUtil::CalcTriangleArea3d(const tVector3 &p0, const tVector3 &p1,
                                    const tVector3 &p2)
{
