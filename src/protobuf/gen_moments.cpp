...
static void generate_moments(const char *fileIn_, const char *fileOut_,
                             int id, sploosh::FMMoments *moment)
{
    char filein[256];
    char fileout[256];
    sprintf(filein, fileIn_, id);
    sprintf(fileout, fileOut_, id);

    FMMBoundIntTransfer<double> tran(filein, fileout);

    // change fileout to new name "moments-%d.dat"
