 ...
 ******************************************************************************/
#ifndef SC_MATRIX_3_INC
#   define SC_MATRIX_3_INC
...
class Matrix3 
{
    ...
    public:
        ...
        T det() const
        {
            return cols[1].cross(cols[2]).dot(cols[0]);
        }

        Matrix3<T> inverse() const
        {
            Vector3<T> v0 = cols[1].cross(cols[2]);
            T det = v0.dot(cols[0]);
            if ( M_ABS(det) < PrecisionType<T>::MA_EPS )
            {
                fprintf(stderr, "ERROR: Invert matrix(1) with det close to 0 %.16lf\n", det);
                return Matrix3<T>::I;
            }
            Vector3<T> v1 = cols[2].cross(cols[0]);
            Vector3<T> v2 = cols[0].cross(cols[1]);
