...
FMMBoundIntTransfer<T>::FMMBoundIntTransfer(
    ...
    const char *fileoutput) : BoundaryIntegralTransfer<T>(fileinput, fileoutput)
{
    ...
    Point3<T> maxPt(-std::numeric_limits<T>::infinity(),
                    -std::numeric_limits<T>::infinity(),
                    -std::numeric_limits<T>::infinity());
    // finding a bounding box, this bounding box will be used to search the location of multipole sources
...
FMMBoundIntTransfer<T>::FMMBoundIntTransfer(const char *fileinput,
                                            const char *fileoutput, const Point3<T> &mc) : objCenter_(mc),
...
void FMMBoundIntTransfer<T>::init()
{
    ...
    else
    {
        ...
        const int MIN_NEXPAN = 16; // if w = 125, nexpan = 16. if w = 125663, nexpan = 40. the number of expan linear increase with omega.
...
void FMMBoundIntTransfer<T>::compute_moments()
{
    ...
    preMoments_.resize(nexpan_ * nexpan_); // nexpan = 20 \in [16, 40]. pre_moments = 400
    memset(&preMoments_[0], 0, sizeof(TComplex) * preMoments_.size());

    for (int i = 0; i < tgl_.size(); ++i)
    {
        Point3<T> sc; // target point to evaluation fundamental solutions
        {
            Point3<T> tc = (vtx_[tgl_[i][0]] + vtx_[tgl_[i][1]] + vtx_[tgl_[i][2]]) * ONE_THIRD; // triangle centroid pos
            cartesian_to_spherical(objCenter_, tc, sc);
        } // convert tc to spherical coords centered at the bounding box center of this object -> sc
        // n \in [0, 20]
        for (int n = 0, cptr = 0; n < nexpan_; cptr += ((++n) << 1))
        { // n=0->1pos; n=1->3pos; n=2->5pos; ...; n=19->39pos. totally 400 pos
            // compute the R^0_n
            ...
            TComplex t = iOmegaRho_ * v_[i]; // (-w * rho * init_velocity, 0) = dp/dn

            /*
            preMoments[] += S* (dp/dn * Basis - p * dBasis/dn)
            如果Basis是格林函数的话, 该公式似乎是边界积分公式. 它计算的就是
            p(x, y) = \int [ dp/dn * G(x, y) + p * dG/dn]
            */
...
void FMMBoundIntTransfer<T>::store_moments(const char *file)
{
    ...
    {
        SIM_ERROR("Cannot write to file: %s\n", file);
    }
