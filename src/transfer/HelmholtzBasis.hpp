...
#include "modal_geometry/Point3.hpp"
...
namespace HelmholtzBasis
{
    ...
    {
        return m & 1 ? -gsl_sf_legendre_sphPlm(n, M_ABS(m), cos(theta)) * std::complex<T>(cos(m * phi), sin(m * phi)) : gsl_sf_legendre_sphPlm(n, M_ABS(m), cos(theta)) * std::complex<T>(cos(m * phi), sin(m * phi));
    }
    ...
    {
        return n < M_ABS(m) ? 0 : (m >= 0 ? (T)(sqrt(double((n - m - 1) * (n - m)) / double((2 * n - 1) * (2 * n + 1)))) : -(T)(sqrt(double((n - m - 1) * (n - m)) / double((2 * n - 1) * (2 * n + 1)))));
    }
    ...
    static std::complex<T> regular_basis(int m, int n, T K, const Point3<T> &sc)
    {
        ...
        return gsl_sf_bessel_jl(n, K * sc.r) * spherical_harmonics(m, n, sc.theta, sc.phi); // the first kind spherical bessel function * spherical harmonics.
    }
