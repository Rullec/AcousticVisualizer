...
public:
    ...
    TComplex eval(const Point3<REAL> &pt) const
    {
        ...
        Point3<REAL> sc; // convert pt into spherical coords
        {
            cartesian_to_spherical(center_, pt, sc);
        } // high order logic

        // calculate sound pressure from many sources centered at AABB midpoint
        for (int n = 0, cptr = 0; n < nexpan_; ++n)
            for (int m = -n; m <= n; ++m)
            {
                TComplex c((*moments_)[cptr][0], (*moments_)[cptr][1]); // moments_ is the coefficient
                ++cptr;
